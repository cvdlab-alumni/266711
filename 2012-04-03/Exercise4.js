//EXERCISE 4, colored 3d model with some table around

//*** in addition to previous jobs, functions that draw an object provides a coloring feature ***
...
var getGlasses = function(){
	...
	var colorFrames = [0.4,0.4,0.4];
	var colorGlasses = [0.78,0.8,1];
	...
	var generateSmallPanels = function(n){
		return COLOR(colorGlasses)(SIMPLEX_GRID([REPLICA(n)([0.95,-0.05]),[0.025],[2.8]]));
	};
	var generateBigPanels = function(n){
		return COLOR(colorGlasses)(SIMPLEX_GRID([REPLICA(n)([3.95,-0.05]),[0.025],[2.8]]));
	};
...
//function that builds a cylinder with some properties
var buildCylinder = function(x){
	var r = x[0];
	var n = x[1];
	var h = x[2];
	var m = x[3];
...
//a function that returns a new instance of table, ready to be placed
var getTable = function(){

	var topTable = T([2])([0.7])(SIMPLEX_GRID([[1.5],[1.5],[0.05]]));
	COLOR([0,0,0])(topTable);

	var leg1 = T([0,1])([0.3,0.3])(buildCylinder([0.08,30,0.7,36]));
	var leg2 = T([0,1])([1.2,0.3])(buildCylinder([0.08,30,0.7,36]));
	var leg3 = T([0,1])([0.3,1.2])(buildCylinder([0.08,30,0.7,36]));
	var leg4 = T([0,1])([1.2,1.2])(buildCylinder([0.08,30,0.7,36]));

	COLOR([0.6,0.6,0.7])(leg1);
	COLOR([0.6,0.6,0.7])(leg2);
	COLOR([0.6,0.6,0.7])(leg3);
	COLOR([0.6,0.6,0.7])(leg4);

	var legs = STRUCT([leg1,leg2,leg3,leg4]);
	var table = STRUCT([topTable,legs]);

	return table;
};

//function that places a mew table centered in the input point
var placeTable = function(point){
	var table = T([0,1,2])([[point[0]-0.75],[point[1]-0.75],[point[2]]])(getTable());
	DRAW(table);
};
